...
import { Menu as MenuIcon, Moon, Sun, FileText } from 'lucide-react'
...
const Header: FC<HeaderProps> = ({ projectsRef, aboutRef, experienceRef }) => {
  ...
  return (
    <nav className="sticky top-0 z-50 mb-12  flex w-full items-center justify-between  bg-light py-4    dark:bg-dark md:w-6/12 ">
      <Logo />
      <div className="flex items-center gap-4">
        <ul className="hidden items-center gap-6 md:flex">
          {REFS.map((item) => (
            <li
              key={item.name}
              className="cursor-pointer px-3 py-1 text-lg font-bold transition-colors duration-300 before:-z-10 hover:bg-secondary "
          ...
          <a
            href="/esteban-cv.pdf"
            download
            className="flex items-center gap-2 border-2 border-black bg-emerald-400 px-3 py-2 font-bold text-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]"
          >
